...
TEST_CASE("PM_material_section_can_be_rewritten_without_re_encoding",
    "[raster][packet]")
{
    using namespace vf::renderer::raster;
    // The mirror patches a material's texture index straight into the encoded
    // packet rather than re-encoding sixty-six megabytes around it, and that
    // is only sound if the materials really are a contiguous section the
    // header locates. If the layout ever changes, this fails here rather than
    // by writing over somebody else's bytes in a live frame.
    DecodedPacket packet{};
    packet.header.width = 256;
    packet.header.height = 192;
    packet.header.frameIndex = 1;
    ...
    packet.header.scissorWidth = 256;
    packet.header.scissorHeight = 192;
    packet.header.vertexSpace = VertexSpace::CameraRelativeWorld;
    ...
    for (std::uint32_t slot = 0; slot < 3; ++slot) {
        RasterVertexV3 vertex{};
        vertex.position[0] = corners[slot][0];
        vertex.position[1] = corners[slot][1];
        vertex.position[2] = corners[slot][2];
        vertex.color[0] = 1.0f;
        packet.vertices.push_back(vertex);
        packet.indices.push_back(slot);
    }
    for (std::uint32_t slot = 0; slot < 4; ++slot) {
        RasterMaterialV1 material{};
        material.resourceId = 0x100 + slot;
        material.baseColor[0] = 1.0f;
        material.textureIndex = kNoMaterialTexture;
        packet.materials.push_back(material);
    }
    RasterDrawV1 draw{};
    draw.materialId = 0x100;
    draw.indexCount = 3;
    packet.draws.push_back(draw);
    ...
    // Change what the mirror changes: one material gains a library entry.
    packet.materials[2].textureIndex = 7;
    std::vector<std::byte> reEncoded;
    REQUIRE(EncodePacket(packet, reEncoded));

    // Patch the section in place, exactly as the mirror does.
    PacketHeaderV1 header{};
    std::memcpy(&header, encoded.data(), sizeof(header));
    REQUIRE(header.materialsOffset != 0);
    REQUIRE(header.materialCount == packet.materials.size());
    const auto bytes = packet.materials.size() * sizeof(RasterMaterialV1);
    REQUIRE(header.materialsOffset + bytes <= encoded.size());
    std::memcpy(encoded.data() + header.materialsOffset,
        packet.materials.data(), bytes);

    // Byte-identical, not merely equivalent: anything else means the patch
    // disturbed a neighbouring section or missed part of its own.
    REQUIRE(encoded.size() == reEncoded.size());
    CHECK(std::memcmp(encoded.data(), reEncoded.data(), encoded.size()) == 0);

    DecodedPacket decoded;
    REQUIRE(DecodePacket(encoded, decoded));
    REQUIRE(decoded.materials.size() == 4);
    CHECK(decoded.materials[2].textureIndex == 7);
    CHECK(decoded.materials[1].textureIndex == kNoMaterialTexture);
    CHECK(decoded.vertices.size() == 3);
}
